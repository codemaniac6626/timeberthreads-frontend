...
export default function ProductList({
  ...
  const cart =
    mode === "cart" ? useAppSelector((state) => state.user.cart) : null;
    ...
    price:
      !!item.product.discountPercent && item.product.discountPercent !== 0
        ? parseFloat(
            (
              item.product.price *
              ((100 - item.product.discountPercent) / 100) *
              item.quantity
            ).toFixed(2),
          )
  ...
  const itemsPrice =
    totalPricePerProduct?.reduce((result, item) => result + item.price, 0) || 0;
  const discountedItemsPrice =
    totalDiscountedPricePerProduct?.reduce(
      (result, item) => result + item.price,
      0,
    ) || 0;

  // Skeleton
  const skeleton = skeletonArray.map((_, idx) => (
  ...
  const errorPic = "https://cdn-icons-png.flaticon.com/512/2832/2832331.png";
            ...
            <Flex direction={{ base: "column", sm: "row" }}>
              ...
              <Stack gap={0} ml={{ base: 0, sm: 20 }} mt={{ base: 20, md: 0 }}>
                ...
                >
                  ...
                  {product.featured === 1 && mode !== "cart" && (
                    <Badge ml={-2} mt={10} size="xs">
                      Featured
                    </Badge>
                  )}
                </Stack>
                  ...
                  {mode !== "cart" ? (
                    ...
                    <>
                      ...
                      {onSale ? (
                        <Text>
                          {"₹" +
                            totalDiscountedPricePerProduct
                              ?.filter(
                                (item) => item.productId === product.productId,
                              )[0]
                              ?.price?.toFixed(2)}
            ...
            <Card.Section pt={20}>
              {onSale && mode !== "cart" ? (
                <Box
                  w={"100%"}
                  h={20}
                  bg={theme.colors.brand[(theme.primaryShade as number) - 2]}
                >
                  <Center pt={2}>
              ...
              <Center>
                ...
                >
                  ...
                  <Stack pt={8} align="center" gap={8}>
                    <Group gap={8}>
                      <Text>Items:</Text>
                      <Text>₹{discountedItemsPrice.toFixed(2)}</Text>
                    </Group>
                    ...
                    <Group gap={8}>
                      <Text>
                        You saved ₹
                        {(itemsPrice - discountedItemsPrice).toFixed(2)} in this
                        order
                      </Text>
                    </Group>
                  </Stack>
